...
library(calendR) # create calendars 
library(glmnet)
library(ggplot2)
library(dplyr)
library(readr)
...
```{r}
df <- cbind(1:10, sort(perm.test$p.val), sort(perm.test.log$p.val))
df <- apply(df, 2, as.numeric)
df <- as.data.frame(df)

perm.vp <- ggplot() +
  geom_point(data = df, aes(x = df[,1], y = df[,2], size = 1)) +
...
perm.vp.log <- ggplot() +
  geom_point(data = df, aes(x = df[,1], y = df[,3], size = 1)) +
  geom_line(aes(x = 0:11, y = 0.005*(0:11), color = "BH"), size = 1)  +
  scale_color_manual(name = "Colors", values = c("BH" = "purple")) +
...
vp.detected <- vibrio[vibrio$Vp_Detect == "1", ]
...
, y = Percent, fill = Vp_Detect2)) +
  ...
  labs(title = "Vibrio Detects by season for all sites", x = "season", y = "Percent") +
  geom_text(data = df.total2 %>% filter(Vp_Detect2=="1") ,mapping = aes(label = paste(n),y = 105)) + 
  labs(fill='Vp Detect')
...
ah.tdh.trh <- vp.detected[, c(45, 46, 47, 48)]
